...
検索ターンでは、遅延を抑えるためにメインのローカルLLMを通さず、Gemini Grounding Search、Gemini Vision Search、Groq Vision、またはWikimedia APIの結果から短い返答を生成します。この場合、Geminiによる感情評価はスキップします。Wikimedia APIの結果は最初にGroqでキャラクター口調へ整形し、Groqが使えない場合はGemini、両方が使えない場合は簡易整形で返します。GeminiとGroqには共通の検索回答用プロンプトを渡し、検索結果やWikipedia本文をそのまま読み上げず、意味を噛み砕いた1〜2文の会話として返します。
